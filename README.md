...
## 同時打鍵について
hoboNicolaライブラリは、かつてのオアシスやASKeyboardといった親指シフトキーボードと同じ同時打鍵ステートマシンを実装しています。
状態遷移とイベント処理の詳細は、[hoboNicolaライブラリ　同時打鍵ステートマシンの実装](./nicola_state.md) を参照してください。
...
## observe_ime について
別プロジェクトで開発している observe_ime.exe (Windows11用) は、入力対象のウィンドウの日本語入力IMEの状態や文字種を読み取り、hoboNicolaデバイスに通知します。

observe_ime については、[こちらを参照](https://github.com/okiraku-camera/observe_ime)。

hoboNicolaデバイスの利用に observe_ime の利用は必須ではありませんが、IMEのオンオフや文字種変更などを hoboNicolaデバイスに依存したくないときに便利です。

hoboNicolaライブラリの1.8.0版から observe_ime のHIDAPIによる通知に対応しています。

### 履歴
* 1.8.0
  * ``WEBHIDやHIDAPI`` と呼ばれる、HIDを通じたホストとデバイス間の通信方法を一部サポートしました。
  * 同時打鍵パラメータの変更
    ...
    * hoboNicola アダプターにも対応しています。rp_hobo_nicola ではフラッシュメモリへの設定書込みのたびにUSB接続が失われるため、操作が煩わしいでしょう。
  * HIDAPIによるNICOLAモードのオンオフ通知
    * observe_ime 1.8.0 は、Windows11のIME状態を検出してNICOLAモードのオンオフを通知します。
    * AVR版のhoboNicolaアダプター、RP2040を使った rp_hobo_nicolaアダプター、RP2040を使ったNK60キーボードにて動作を確認しています。
  
  * このバージョンでは、キーコードのカスタマイズ機能は公開していません。
  * ライブラリ等の依存関係は1.7.9と同じ。
