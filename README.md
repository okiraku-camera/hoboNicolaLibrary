...
## 親指キーについて
hoboNicolaは、アダプターに接続したキーボードから出力されるキーコードや、hoboNicolaを組み込んだキーボード(hoboNicolaキーボード)の基本配列に定義されているコードに基づいて、上図のような配列への変更や親指キー同時打鍵処理を行います。左右の親指キーに割り当てることができるキーは以下のようになっており、を単独打鍵したときにこれらのキーのコードが出力されます。
...
アダプターの場合、接続しているキーボードの親指キーのコードを、USレイアウトならばF14やF15にカスタマイズする必要があります。

MacOSで使う場合、Windowsの変換、無変換、ひらがなといったキーは無視されます。hoboNicolaでは、ひらがなキーはImeOn (HID Lang1)、英数キーはImeOff (HID Lang2) とすることができるので、たとえば Windows用日本語キーボードの「ひらがな」と「英数」キーにそれらを割り当てておけば快適です。変換キーは空白キーで、無変換キーはF14キーなどで代用できますし、Enterキーとすることもできます。

Ubuntu 24.4 で fcitx5 + mozc での組み合わせで使う場合、入力ソースとしてのmozcのオンオフにImeOn, ImeOffを利用できます。無変換、変換キーについてはWindows用キーボードと同じコードでよいようです。
...
* Visual Studio Code(1.90), Arduino-extension 0.6以降 とarduino-cliが導入済みであること。
* Arduino IDE (2.x.x) でもビルドできると思いますが確認していません。

いくつかのUSBホストコントローラー用の実装やキーボードPCB用の実装は Arduino スケッチ (.inoファイル) として examples ディレクトリに入っているので、ターゲットとするデバイスやマイコンに応じたスケッチを開き、別名で保存するところからビルドしたり変更したりを始めることができます。
