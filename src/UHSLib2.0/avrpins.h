...
// 1.5.0 add ISP1807MB
// 1.6.2 add XIAO BLE
#elif defined(ARDUINO_SSCI_ISP1807_MICRO_BOARD)	|| defined(ARDUINO_Seeed_XIAO_nRF52840)
...
#if defined(ARDUINO_SSCI_ISP1807_MICRO_BOARD)	// ISP1807 MB
...
MAKE_PIN(P15, (45)); // SCK (P1.13)
MAKE_PIN(P14, (46)); // MISO (P1.14) 
MAKE_PIN(P16, (47)); // MOSI (P1.15)
MAKE_PIN(P10, (4)); // SS (D4) (P0.4)
MAKE_PIN(P9, (5));	// INT (D5)  (P0.5)
#undef MAKE_PIN
#else
#error "Please define nRF52840 board in avrpins.h"
#endif

// 1.6.1 add SAMD21 boards.
#elif defined(ADAFRUIT_QTPY_M0) || defined(SEEED_XIAO_M0)

#if 0
// PORT_PINCFG_INENを出力側にセットするのは出力値を読めるようにするため。
#define MAKE_PIN(className, port, pin) \
class className { \
public: \
  static void Set() { \
  ...
  static void SetDirWrite() { \
    PORT->Group[port].PINCFG[pin].reg=(uint8_t)(PORT_PINCFG_INEN) ; \
    PORT->Group[port].DIRSET.reg = (1ul << pi); \
  } \
  static uint8_t IsSet() { \
    return PORT->Group[port].IN.reg & (1ul << pin); \
  } \
};
// Pxx の数字はisp1807に合わせただけ。
...
// 1.6.2 add rp2040 boards (Arduino-pico core)
...
// Pxx の数字はisp1807に合わせた。
MAKE_PIN(P15, 2); // SCK(SCK / GPIO2)
MAKE_PIN(P14, 4); // MISO(MI / GPIO4)
MAKE_PIN(P16, 3); // MOSI(MO / GPIO3)
MAKE_PIN(P9,  7); // INT (SCL / GPIO7 = D5)
MAKE_PIN(P10, 6); // SS  (SDA / GPIO6 = D4)
#undef MAKE_PIN
#elif defined(ARDUINO_ADAFRUIT_KB2040_RP2040)
MAKE_PIN(P15, 18); // SCK(SCK / GPIO18)
MAKE_PIN(P14, 20); // MISO(MI / GPIO20)
MAKE_PIN(P16, 19); // MOSI(MO / GPIO19)
MAKE_PIN(P10, 10); // SS  (D10 / GPIO10)
MAKE_PIN(P9,  9); // INT (D9 / GPIO9)
#undef MAKE_PIN
#elif defined(ARDUINO_RASPBERRY_PI_PICO)
// 1.6.3 add rp-pico
MAKE_PIN(P15, 18); // SCK(SCK / GPIO18)
MAKE_PIN(P14, 16); // MISO(MI / GPIO16)
MAKE_PIN(P16, 19); // MOSI(MO / GPIO19)
MAKE_PIN(P10, 17); // SS  (GPIO17)
MAKE_PIN(P9,  20); // INT (GPIO20)
#undef MAKE_PIN
...
#else
#error "Please define rp2040 pin configs in avrpins.h"
#endif
