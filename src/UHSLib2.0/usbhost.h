...
/* SPI initialization */
template< typename UHS_SPI_CLK, typename UHS_SPI_MOSI, typename UHS_SPI_MISO, typename UHS_SPI_SS > class SPi {
public:
#if USING_SPI4TEENSY3
        static void init() {
...
#warning "You need to initialize the SPI interface manually when using the STM32F4 platform"
        ...
        static void init() {
                //uint8_t tmp;
                UHS_SPI_CLK::SetDirWrite();
                UHS_SPI_MOSI::SetDirWrite();
                UHS_SPI_MISO::SetDirRead();
                UHS_SPI_SS::SetDirWrite();
...
#if defined(PIN_SPI_SCK) && defined(PIN_UHS_SPI_MOSI) && defined(PIN_UHS_SPI_MISO) && defined(PIN_UHS_SPI_SS)
...
typedef SPi< MAKE_PIN(PIN_SPI_SCK), MAKE_PIN(PIN_UHS_SPI_MOSI), MAKE_PIN(PIN_UHS_SPI_MISO), MAKE_PIN(PIN_UHS_SPI_SS) > spi;
#undef MAKE_PIN
...
#elif defined(ARDUINO_SSCI_ISP1807_MICRO_BOARD) ||  defined(ADAFRUIT_QTPY_M0) || defined(SEEED_XIAO_M0) || defined(ARDUINO_Seeed_XIAO_nRF52840) || defined(ARDUINO_SEEED_XIAO_RP2040) || defined(ARDUINO_SEEED_XAIO_RP2040) || defined(ARDUINO_RASPBERRY_PI_PICO) || defined(ARDUINO_ADAFRUIT_KB2040_RP2040)
typedef SPi< P15, P16, P14, P10 > spi;
#elif defined(ARDUINO_GENERIC_RP2040) 
typedef SPi< P15, P16, P14, P10 > spi;	// dummy definition.
...
template< typename UHS_SPI_SS, typename INTR > class MAX3421e /* : public spi */ {
...
template< typename UHS_SPI_SS, typename INTR >
        uint8_t MAX3421e< UHS_SPI_SS, INTR >::vbusState = 0;

/* constructor */
template< typename UHS_SPI_SS, typename INTR >
MAX3421e< UHS_SPI_SS, INTR >::MAX3421e() {
...
#elif defined(ARDUINO_NRF52_ADAFRUIT) || defined(ARDUINO_ARCH_RP2040)
...
template< typename UHS_SPI_SS, typename INTR >
void MAX3421e< UHS_SPI_SS, INTR >::regWr(uint8_t reg, uint8_t data) {
        XMEM_ACQUIRE_SPI();
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t* MAX3421e< UHS_SPI_SS, INTR >::bytesWr(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
        XMEM_ACQUIRE_SPI();
#if defined(SPI_HAS_TRANSACTION)
        USB_SPI.beginTransaction(SPISettings(SPI_CLOCK, MSBFIRST, SPI_MODE0)); // The MAX3421E can handle up to 26MHz, use MSB First and SPI mode 0
#endif
...
template< typename UHS_SPI_SS, typename INTR >
void MAX3421e< UHS_SPI_SS, INTR >::gpioWr(uint8_t data) {
        regWr(rIOPINS1, data);
        data >>= 4;
        regWr(rIOPINS2, data);
        return;
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t MAX3421e< UHS_SPI_SS, INTR >::regRd(uint8_t reg) {
        XMEM_ACQUIRE_SPI();
#if defined(SPI_HAS_TRANSACTION)
        USB_SPI.beginTransaction(SPISettings(SPI_CLOCK, MSBFIRST, SPI_MODE0)); // The MAX3421E can handle up to 26MHz, use MSB First and SPI mode 0
#endif
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t* MAX3421e< UHS_SPI_SS, INTR >::bytesRd(uint8_t reg, uint8_t nbytes, uint8_t* data_p) {
        XMEM_ACQUIRE_SPI();
#if defined(SPI_HAS_TRANSACTION)
        USB_SPI.beginTransaction(SPISettings(SPI_CLOCK, MSBFIRST, SPI_MODE0)); // The MAX3421E can handle up to 26MHz, use MSB First and SPI mode 0
#endif
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t MAX3421e< UHS_SPI_SS, INTR >::gpioRd() {
        uint8_t gpin = 0;
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t MAX3421e< UHS_SPI_SS, INTR >::gpioRdOutput() {
        uint8_t gpout = 0;
...
template< typename UHS_SPI_SS, typename INTR >
uint16_t MAX3421e< UHS_SPI_SS, INTR >::reset() {
        uint16_t i = 0;
...
template< typename UHS_SPI_SS, typename INTR >
int8_t MAX3421e< UHS_SPI_SS, INTR >::Init() {
        XMEM_ACQUIRE_SPI();
        // Moved here.
...
/* initialize MAX3421E. Set Host mode, pullups, and stuff. Returns 0 if success, -1 if not */
template< typename UHS_SPI_SS, typename INTR >
int8_t MAX3421e< UHS_SPI_SS, INTR >::Init(int mseconds) {
        XMEM_ACQUIRE_SPI();
        // Moved here.
        // you really should not init hardware in the constructor when it involves locks.
        // Also avoids the vbus flicker issue confusing some devices.
...
template< typename UHS_SPI_SS, typename INTR >
void MAX3421e< UHS_SPI_SS, INTR >::busprobe() {
        uint8_t bus_sample;
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t MAX3421e< UHS_SPI_SS, INTR >::Task(void) {
        uint8_t rcode = 0;
        uint8_t pinvalue;
...
template< typename UHS_SPI_SS, typename INTR >
uint8_t MAX3421e< UHS_SPI_SS, INTR >::IntHandler() {
        uint8_t HIRQ;
...
//template< typename UHS_SPI_SS, typename INTR >
//uint8_t MAX3421e< UHS_SPI_SS, INTR >::GpxHandler()
//{
