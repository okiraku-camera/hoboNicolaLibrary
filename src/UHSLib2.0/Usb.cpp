...
// resume request 
// 
bool USBHost::resumeFromKeyboard() {
	bool f = (regRd(rHIRQ) & bmRSMREQIRQ) != 0;
	if (f)
		regWr(rHIRQ, bmRSMREQIRQ);
	return f;
}

bool USBHost::resumeRequest() {
	uint8_t v = regRd(rHIRQ);
	return (v & bmRCVDAVIRQ);
}
#endif

void USBHost::suspendKeyboard() {
	uint8_t mode = regRd(rMODE);
	mode &= ~bmSOFKAENAB;
	regWr(rMODE, mode);	// stop SOF or KeepAlive 
	delay(5);
	regWr(rHIRQ, bmSUSDNIRQ);
	delay(10);
	regWr(rUSBCTL, bmPWRDOWN);	// Enter powerdown state of MAX3421

}

void USBHost::resumeKeyboard() {
#if 0
	regWr(rHCTL, bmSIGRSM); // 
	while(!(regRd(rHCTL) & bmSIGRSM));	// wait for SIGRSM is cleared.
//	while(!(regRd(rHIRQ) & bmBUSEVENTIRQ)); //wait for sample operation to finish
#endif
	regWr(rUSBCTL, 0);
	delay(10);	// OSCOKIRQをテストするのが正しい。
	uint8_t mode = regRd(rMODE);
	mode |= bmSOFKAENAB;
	regWr(rMODE, mode);
	delay(10);
